...
Design Tokens
: Named entities that store visual design attributes—colors, spacing, typography—so they can be maintained in a single source of truth.
...
**The Why and When**: Use the rule above to provide breathing room between unrelated blocks of content, or to mark a transition where a new heading would feel over-prescriptive and too heavy.
...
**The Why and When**: Use footnotes for tangential commentary, citations, or technical asides that would interrupt the flow of the main argument. They provide a "layer" of depth for the curious reader without blocking the path for casual readers. Multiple footnotes accumulate naturally[^3].
